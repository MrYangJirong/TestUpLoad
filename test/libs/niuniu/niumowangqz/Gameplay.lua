...
function Gameplay:findBigBanker()
  ...
  local rdPlayer = self.game.players[math.random(#self.game.players)]
  local max = rdPlayer
  local maxValue = rdPlayer.hand.qiangCnt
...
function PutMoney:on(p, e)
  ...
  if table.empty(self.puts) then
    self:broadcast( self.gName..'.putOver' )
