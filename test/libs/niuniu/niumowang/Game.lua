...
function Game:compareResult()
	if self.option.gameplay == 5 then
		for i = 1, #self.players - 1 do
			for si = i, #self.players do
				self:compare(self.players[i], self.players[si])
			end
		end
	else
...
function Game:updateMoney()
	...
	for _, v in ipairs(self.players) do
		...
		-- 牛牛上庄更新庄家
		if self.option.gameplay == 1 then
			print("		玩家:", v:getNickname(), "niuCnt:", v.hand.niuCnt)
			if v.hand.niuCnt == 10 and v.hand.specialType == 0 and self.summary[uid].score > 0 then
				if nil == niuniuPlayer then
					niuniuPlayer = v
					self:updateBanker(v)
					print("		========> 牛牛上庄1:", v:getNickname(), niuniuPlayer:getNickname(), v.hand.niuCnt)
				elseif niuniuPlayer and self:bigThan(v, niuniuPlayer) then
					niuniuPlayer = v
					self:updateBanker(v)
					print("		========> 牛牛上庄2:", v:getNickname(), niuniuPlayer:getNickname(), v.hand.niuCnt)
				end
			end
		end
	...
	if self.option.gameplay == 1 and not niuniuPlayer then
		self:updateBanker(self.banker)
	end
